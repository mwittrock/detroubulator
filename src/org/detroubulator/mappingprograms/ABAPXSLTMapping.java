...
public final class ABAPXSLTMapping extends AbstractMapping {

    private static Logger log = Logging.getLogger(ABAPXSLTMapping.class);
    ...
    public void configure(Map<String, List<String>> params)
        ...
        log.entering(ABAPXSLTMapping.class.getName(), "configure", params);
        if (params == null) {
        ...
        configured = true;
        log.exiting(ABAPXSLTMapping.class.getName(), "configure");
    }

    public MappingOutput execute(TransformationParams params, MappingInput input)
            throws ServerException {
        log.entering(ABAPXSLTMapping.class.getName(), "execute", new Object[] {
                params, input });
        if (!canExecute()) {
        ...
        MappingOutput result = getServer().executeABAPXSLTMapping(this, input,
                params);
        log.exiting(ABAPXSLTMapping.class.getName(), "execute", result);
        return result;
    }

    public String toString() {
        log.entering(ABAPXSLTMapping.class.getName(), "identify");
        String ident = String.format("ABAP XSLT mapping %s", getMappingName());
        log.exiting(ABAPXSLTMapping.class.getName(), "identify", ident);
        return ident;
    }
