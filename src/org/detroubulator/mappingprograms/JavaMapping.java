...
public class JavaMapping extends AbstractMapping {

    private static Logger log = Logging.getLogger(JavaMapping.class);
    ...
    public boolean isConfigured() {
        log.entering(JavaMapping.class.getName(), "isConfigured");
        log.exiting(JavaMapping.class.getName(), "isConfigured", configured);
        return configured;
    }

    public void configure(Map<String, List<String>> params) throws ConfigurationException {
        log.entering(JavaMapping.class.getName(), "configure", params); 
        if (params == null) {
        ...
        configured = true;
        log.exiting(JavaMapping.class.getName(), "configure");
    }

    public MappingOutput execute(TransformationParams params, MappingInput input) throws ServerException {
    	log.entering(JavaMapping.class.getName(), "execute", new Object[] {params, input});
    	if (!canExecute()) {
    		IllegalStateException e = new IllegalStateException("No associated server");
    		log.throwing(this.getClass().getName(), "execute", e);
    		throw e;
    	}
        MappingOutput result = getServer().executeJavaMapping(this, input, params);
        log.exiting(JavaMapping.class.getName(), "execute", result);
        return result;
    }

    public String toString() {
        log.entering(JavaMapping.class.getName(), "identify");
        String ident = String.format("Java mapping %s", getMappingName());
        log.exiting(JavaMapping.class.getName(), "identify", ident);
        return ident;
    }
