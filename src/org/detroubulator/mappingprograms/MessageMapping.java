...
public final class MessageMapping extends AbstractMapping {
    ...
    public boolean isConfigured() {
        log.entering(MessageMapping.class.getName(), "isConfigured");
        log.exiting(MessageMapping.class.getName(), "isConfigured", configured);
        return configured;
    }

    public void configure(Map<String, List<String>> params) throws ConfigurationException {
        log.entering(MessageMapping.class.getName(), "configure", params); 
        if (params == null) {
        ...
        configured = true;
        log.exiting(MessageMapping.class.getName(), "configure");
    }

    public MappingOutput execute(TransformationParams params, MappingInput input) throws ServerException {
    	log.entering(MessageMapping.class.getName(), "execute", new Object[] {params, input});
    	if (!canExecute()) {
    		IllegalStateException e = new IllegalStateException("No associated server");
    		log.throwing(this.getClass().getName(), "execute", e);
    		throw e;
    	}
        MappingOutput result = getServer().executeMessageMapping(this, input, params);
        log.exiting(MessageMapping.class.getName(), "execute", result);
        return result;
    }

    public String toString() {
        log.entering(MessageMapping.class.getName(), "identify");
        String ident = String.format("Message mapping %s", getMappingName());
        log.exiting(MessageMapping.class.getName(), "identify", ident);
        return ident;
    }
